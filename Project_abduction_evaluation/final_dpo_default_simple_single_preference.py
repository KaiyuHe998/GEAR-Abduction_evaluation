...
def print_split_headcounts(structured_data: Dict[str, Any]) -> None:
    for ds_name, io_counts in structured_data.items():
        ...
        print(f"🔍 Dataset '{ds_name}' unique problem count: {len(pid_set)}")
...
def create_problem_based_splits(
    ...
    """Original splitter, unchanged except it removes curriculum bits.
    ...
    if use_external_split:
        print("⚡ Using external splits (train/test/leftover).")
        ...
        print(f"ℹ️ Global minimum train set size: {min_train}, global minimum test set size: {min_test}")
    ...
    print("✅ Train/test split completed based on external/automatic partition!")
...
def sample_training_data_balanced(
    ...
    - If a dataset lacks enough examples for its quota, redistribute the remainder to other datasets of the *same reason*.
    - If the reason as a whole has fewer examples than its quota, take all from that reason (no cross-reason top-up).
    """
    ...
    for reason in reasons:
        ...
        if not per_ds:
            print(f"  ⚠️ Reason '{reason}' does not exist in the original data. Skipping.")
            continue

        target = quotas[reason]
        avail_total = sum(len(v) for v in per_ds.values())
        if avail_total <= target:
            print(f"  ⚠️ Insufficient data for reason '{reason}': only {avail_total} available, target {target}. Taking all.")
        ...
        print(f"  ✅ Sampled {target} for '{reason}' (per-dataset allocation: "
              + ", ".join([f"{ds}:{alloc[ds]}" for ds in sorted(ds_keys)]) + ")")

    rng.shuffle(sampled)
    print(f"\n✅ Sampling complete. Total {len(sampled)} examples.")
    return sampled
...
if __name__ == '__main__':
    ...
    print(f"\n📊 Post-split sizes:\n  - Train (full): {len(full_train_data)}\n  - Test (full): {len(test_data)}")
    ...
    class CustomDPOTrainer(DPOTrainer):
        def evaluate(
            ...
            for reason in ['parsing', 'inconsistent', 'fair comparsion']:
                ...
                # Step 1: Filter (and cache) the dataset by reason
                ...
                assert len(reason_dataset) != 0, f'''Reason-specific dataset is empty after filtering'''
    
                # Step 2: Choose subset to evaluate
                # Note: for datasets objects, use .shuffle().select() rather than random.sample
                ...
                # DPOTrainer returns loss under 'loss', which becomes 'eval/category/loss' with our prefix
                loss_key_for_reason = f"{current_metric_prefix}_loss"
    
                # Step 3: Call parent evaluate for this subset
                ...
                # Step 4: Collect metrics and compute weighted-loss contribution
                if loss_key_for_reason in reason_metrics:
                    ...
                    # Merge namespaced metrics
                    all_metrics.update(reason_metrics)
                else:
                    print(f"  ⚠️  Warning: Loss key '{loss_key_for_reason}' not found in metrics for reason '{reason}'. Existing keys: {reason_metrics.keys()}.")
                    all_metrics.update(reason_metrics)

                self.custom_metrics_history[reason].append(reason_metrics)

            # Step 5: Final weighted loss
            ...
            if self.control.should_training_stop:  
                ...
                print(f'Current early-stop state: {self.control.should_training_stop}. Continue training.')
